...
```{r}
setwd("..")
partitions = read_csv("data/partitions.csv")%>%
...
nodes_info =cbind(nodes_good,nodes_bad)
...
```{r}
setwd("..")
ggplot(data = new_genres %>% filter(perc > .1), aes(x = genre,y = count, fill = community)) + 
  geom_col() + 
  facet_wrap(~community, nrow = 4, scales = "free_y") +
  scale_fill_brewer(type = "qual", palette = "Paired", name = "Community") +
  labs(
    title = "Genres Within Communities",
    x = "Genre",
    y = "Count"
    ) +
  theme(plot.title = element_text(
